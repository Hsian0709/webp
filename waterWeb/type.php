...
if ($watertype == "水庫湖泊") {
    while ($content = mysqli_fetch_assoc($result)) {
        ...
        $pokemon_table .= "<li style='padding: 5px 5px;'><a href={$page}{$name} style='color:black;font-size:large;' >" . $name . "</a></li>";
    }
} elseif ($watertype == "溪河") {
    while ($content = mysqli_fetch_assoc($result)) {
        $HP = $content["HP"];
        $pokemon_table .= "<li style='padding: 5px 5px;'><a href={$page}{$HP} style='color:black;font-size:large;' >" . $HP . "</a></li>";
    }
} elseif ($watertype == "海岸") {
    while ($content = mysqli_fetch_assoc($result)) {
        $attack = $content["Attack"];
        $pokemon_table .= "<li style='padding: 5px 5px;'><a href={$page}{$attack} style='color:black;font-size:large;' >" . $attack . "</a></li>";
    }
} elseif ($watertype == "漁港") {
    while ($content = mysqli_fetch_assoc($result)) {
        $def = $content["Defense"];
        $pokemon_table .= "<li style='padding: 5px 5px;'><a href={$page}{$def} style='color:black;font-size:large;' >" . $def . "</a></li>";
    }
}
